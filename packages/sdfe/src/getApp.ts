...
import { IS_MONOREPO, MONOREPO_DIR, PROCESS_CWD } from './constants'
...
export default async function (options: Record<string, any>): Promise<string[]> {
  ...
  if (options.app?.length > 0) {
    ...
    if (options.app.map((item: string) => item.toLowerCase()).includes('all')) {
      return apps.map((app: string) => resolve(PROCESS_CWD, MONOREPO_DIR, app))
    }
    ...
    // 返回选中app
    const appsPath = res.selectedApps?.map((app: string) => resolve(PROCESS_CWD, MONOREPO_DIR, app))
